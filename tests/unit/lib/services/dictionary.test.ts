...
describe('Dictionary tests', () => {
  ...
  test('Returns initial batch of pages to build', () => {
    ...
    expect(wordsToBuild.length).toEqual(5964)

    // Deterministic entry slugs, roughly spread through dictionary.
    expect(wordsToBuild[0]).toEqual('a')
    expect(wordsToBuild[10]).toEqual('adhal-vark')
    expect(wordsToBuild[100]).toEqual('alhalghona-dagher')
    expect(wordsToBuild[1000]).toEqual('folk-2')
    expect(wordsToBuild[2000]).toEqual('hastbaker-2')
    expect(wordsToBuild[3000]).toEqual('mishopa')
    expect(wordsToBuild[4000]).toEqual('saman-falla-3')
    expect(wordsToBuild[5000]).toEqual('thval')
    expect(wordsToBuild[5900]).toEqual('ortogh-2')
  })
})
